...
def guess_is_word(words, guess, score, guess_word, pattern, correct_letters, chosen_letters, wrong_guess_lst, msg):
    """
    ...
    if guess[0] == WORD:
        ...
        for i in range(len(guess[1])):
            ...
            if word[i] in wrong_guess_lst or len(word) != len(guess_word):
                msg = ''
        ...
        if word == pattern:
            ...
            #display_state(pattern, wrong_guess_lst, score, msg)
        if score == 0:
            print("you lost the game. the word was " + guess_word + " .")
...
def guess_is_hint(words,guess, pattern, score, correct_letters, wrong_guess_lst, chosen_letters, guess_word, msg):
    """
...
def run_single_game(words, score):
    ...
    while score > 0 and pattern != guess_word:
        ...
        score, pattern, correct_letters, chosen_letters , msg = guess_is_word\
            (words, guess, score, guess_word, pattern,
             correct_letters, chosen_letters, wrong_guess_lst, msg)
        ...
        score, pattern ,msg = guess_is_hint(words, guess, pattern, score,
               correct_letters, wrong_guess_lst, chosen_letters, guess_word, msg)
        display_state(pattern, wrong_guess_lst, score, msg)
    return score
